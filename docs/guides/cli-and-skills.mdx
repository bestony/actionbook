...
2.  If `actionbook get` already covers required fields, go directly to script generation.
3.  Use snapshot/screenshot/inspect only when selectors are missing or fail in validation.
4.  Generate a **standalone Playwright script** that handles detected page mechanisms.
5.  Run the script and write the extracted data to disk.
