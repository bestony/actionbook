...
The `extract` skill handles requests where the goal is **obtaining data** from a website. Ask naturally:
...
The agent will:
1.  Search Actionbook for verified selectors (or fall back to snapshot/screenshot).
2.  Detect page mechanisms (lazy loading, virtual scroll, pagination).
3.  Generate a **standalone Playwright script** that handles those mechanisms.
4.  Run the script and write the extracted data to disk.
