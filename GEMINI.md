...
actionbook/
├── apps/
│   ├── api-service/        # REST API (Next.js App Router, Vercel deployment)
│   ├── website/            # Main Website (Next.js 16)
│   ├── api-server/         # (Placeholder/Future Use)
...
│   └── tools-ai-sdk/       # AI SDK integration tools
├── services/
...
API_SERVICE_URL=https://...        # API Service URL for MCP
MCP_TRANSPORT=stdio                # stdio | sse
```

## Development Workflow

1. This is a pnpm workspace - always use `pnpm` instead of `npm` or `yarn`
2. Node.js 20+ and pnpm 10+ are required
3. Check for existing CLAUDE.md files in subdirectories for package-specific guidance
4. Follow existing code patterns and conventions in each workspace
5. Environment variables are configured per workspace - check local README files
...
[apps/api-service]feat: add new container management endpoint
...
- **API Server**: Next.js serverless functions providing REST API
- **Database**: PostgreSQL for API key management and state
