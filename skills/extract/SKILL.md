...
1. **Executable Playwright script** — a standalone `.js` file that reproduces the extraction without Actionbook at runtime.
...
while (true) {
  ...
  await Promise.all([
    // Some sites navigate, some update client-side only
    page.waitForLoadState('networkidle').catch(() => {}),
    nextBtn.click(),
  ]);

  // If URL doesn't change (SPA), wait for list to update instead
...
Write a standalone Playwright script (`extract_<domain>_<slug>.js`) that:
...
```javascript
// extract_<domain>_<slug>.js
// Generated by Actionbook extract skill
// Usage: node extract_<domain>_<slug>.js
...
```bash
node extract_<domain>_<slug>.js
```
...
Present to the user:
1. **Script path** — the `.js` file they can re-run anytime.
...
|----------|------|--------|
| Playwright script | `./extract_<domain>_<slug>.js` | Standalone Node.js script using `playwright` |
